...
様々なプログラミング言語が存在します。C、C++、Java、JavaScript、Rust、Go、Python, Ruby、[Standard ML][sml]、その他。
...
$L$というプログラミング言語があるとすれば、$L$言語の実装は、$p$という$L$言語のプログラムをインプットとして、$p$による何らかのアウトプットを作り出すプログラムです。
...
次は動的意味論、つまりどうプログラムを評価するか、を定義する番です。動的意味論を定義する方法はいくつかありますが、私達は[構造操作的意味論][struct-op-sem]にします。
...
そして、$e_1$が値だったらどうするか定義します。静的意味論では$e_1$の型はブーリアンなので、値だったら絶対に$\mathtt{true}$か$\mathtt{false}$ですね。（これを後で証明するぜ！）
...
これで発言の動的意味論の定義も完了したのです。

## 定理

前に、形式手法で言語を仕様すればその仕様についての定理を陳述し証明できると言いましたね。では定理を陳述しようではありませんか。
