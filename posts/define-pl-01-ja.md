...
この投稿のシリーズでは、私達は小さなプログラミング言語を公式な方法で定義します。その定義について[Lean][lean]を使い定理を証明します。
...
前に、公式な方法で言語を仕様すればその仕様についての定理を陳述し証明できると言いましたね。では定理を陳述しようではありませんか。
