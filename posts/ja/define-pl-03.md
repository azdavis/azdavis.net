...
子の投稿では、積型を追加します。本格的なプログラミング言語では、積は struct、record、tuple などでも呼ばれています。
...
次は型を二つ重ねる「ペア」という積型です。$\tau_1$と$\tau_2$は型だとしたら、$\tau_1 \times \tau_2$はそれらを合わせるペア型です。そして、$e_1$の型が$\tau_1$で$e_2$の型が$\tau_2$だとすれば、$\langle e_1, e_2 \rangle$は$\tau_1 \times \tau_2$の型のあるぺは定数です。

ペアを使用する為、中身の値を抽出せねば。その為$e.\mathsf{left}$と$e.\mathsf{right}$の式も追加します。$e$は積であれば、この式は左と右の値を積から抽出します。
